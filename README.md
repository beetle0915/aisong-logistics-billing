...
- 读取业务员对应的快递报价表。
- 自动解析 `快递公司（标准版）`。
- 规则配置中强化快递公司映射说明，帮助维护销售表名称到报价表 sheet 的对应关系。
- 对报价目录、销售表结构、重量和报价匹配异常输出更明确的处理建议。
- 支持顺丰、德邦 20kg 以上大件模板。
- 生成总结果 Excel。
- 按业务员拆分客户每日明细。
- 生成客户历史汇总表，文件名带业务员名称，含历史快递明细、收款记录和余额公式。
- 生成选项按钮使用明确的 `✅` / `□` 状态标识。
- 提供 Tkinter 桌面 GUI。
- 支持 macOS `.app` 打包。
...
docs/
  ...
  PRD_艾松物流计费系统_V7_0_6_历史快递明细增量覆盖.md
  PRD_艾松物流计费系统_V7_0_7_关键异常日志优化.md
  PRD_艾松物流计费系统_V7_0_8_生成选项状态标识优化.md
  PRD_艾松物流计费系统_V7_0_9_快递公司映射配置优化.md
  说明_当前快递费用计算逻辑.md
.github/workflows/
  build-windows.yml
```
...
```text
aisong-logistics-billing-v7.0.9-windows
```
